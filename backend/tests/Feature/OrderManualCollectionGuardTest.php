...
class OrderManualCollectionGuardTest extends TestCase
{
    ...
    public function test_should_not_mark_fully_collected_when_shipping_receivable_remains(): void
    {
        $order = Order::withoutEvents(function () {
            $order = Order::create([
                'customer_name' => 'جزئي شحن',
                'customer_type' => 'فرد',
                'customer_phone_1' => '01012345678',
                'customer_phone_2' => '',
                'governorate' => 'القاهرة',
                'city' => 'القاهرة',
                'address' => 'عنوان',
        ...
        $this->assertFalse($guard->shouldMarkOrderFullyCollected($order));
        $this->assertFalse(Order::reconcileCollectionStatusIfAllShippingLinesClosed((int) $order->id));
        $this->assertSame('تم التسليم', $order->fresh()->order_status);
    }

    public function test_should_not_mark_fully_collected_when_only_reopened_shipping_lines_exist(): void
    {
        $order = Order::withoutEvents(function () {
            $order = Order::create([
                'customer_name' => 'إعادة فتح',
                'customer_type' => 'فرد',
                'customer_phone_1' => '01087654321',
                'customer_phone_2' => '',
                'governorate' => 'القاهرة',
                'city' => 'القاهرة',
                'address' => 'عنوان',
