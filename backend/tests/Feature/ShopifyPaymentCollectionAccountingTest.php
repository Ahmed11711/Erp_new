...
class ShopifyPaymentCollectionAccountingTest extends TestCase
{
    ...
    public function test_paid_shopify_order_posts_receivable_on_matching_collection_company(): void
    {
        ...
        $advanceEntries = AccountEntry::where('order_id', $order->id)
            ->where('entry_batch_code', 'like', 'ORD-PREPAID-' . $order->id . '-%')
            ->get();
        $this->assertSame(2, $advanceEntries->count(), 'Advance journal: Dr collection / Cr customer');
        $this->assertEquals(1050.0, (float) $advanceEntries->sum('debit'));
        $this->assertEquals(0.0, (float) $advanceEntries->where('tree_account_id', $collectReceivableId)->sum('credit'));
