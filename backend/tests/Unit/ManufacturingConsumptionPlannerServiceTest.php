...
class ManufacturingConsumptionPlannerServiceTest extends TestCase
{
    ...
    public function test_preview_ignores_stale_unit_quantities_when_batch_qty_is_scaled(): void
    {
        DB::beginTransaction();
        [$finished, $fabric] = $this->makeFinishedWithRawBom(2, 10);

        $preview = app(ManufacturingConsumptionPlannerService::class)->preview((int) $finished->id, 3, [[
            'bom_item_id' => (int) $fabric->id,
            'resolved_category_id' => (int) $fabric->id,
            'quantity' => 2,
        ]]);
        ...
        $this->assertTrue($preview['applies']);
        $this->assertEqualsWithDelta(6.0, (float) $preview['lines'][0]['quantity'], 0.0001);
        $this->assertFalse($preview['lines'][0]['is_customized']);
    }

    public function test_preview_keeps_intentionally_customized_quantities(): void
    {
        DB::beginTransaction();
        [$finished, $fabric] = $this->makeFinishedWithRawBom(2, 10);
        ...
        $this->assertEqualsWithDelta(5.0, (float) $preview['lines'][0]['quantity'], 0.0001);
        $this->assertTrue($preview['lines'][0]['is_customized']);
    }
    ...
    private function makeFinishedWithRawBom(float $bomQty, float $unitCost): array
    {
        ...
        $fabric = Category::create([
            'category_name' => 'Fabric_'.uniqid(),
