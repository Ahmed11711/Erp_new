...
class OrderLiabilityTransferServiceTest extends TestCase
{
    ...
    private function service(): OrderLiabilityTransferService
    {
        return new OrderLiabilityTransferService(
            ...
            new \App\Services\Shipping\ShippingReceivableSplitService(),
        );
    }
    ...
    public function test_mixed_online_cash_split_and_delivery_holder_is_courier(): void
    {
        $order = new Order([
            'net_total' => 200,
            'prepaid_amount' => 3842,
        ...
        $od = new OrderDetails([
            ...
            'collection_receivable_amount' => 3842,
            'shipping_receivable_amount' => 200,
            'collection_provider_type' => 'collection_company',
            'collection_provider_id' => 3,
            'shipping_provider_id' => 55,
            'shipping_company_id' => 55,
        ]);
        $od->setRelation('shipping_company', new \App\Models\ShippingCompany([
            'id' => 55,
            'name' => 'الظافر',
            'type' => 'مندوب',
        ]));
        ...
        $split = $this->service()->mixedOnlineCashSplit($order, $od);

        $this->assertNotNull($split);
        $this->assertEqualsWithDelta(3842.0, $split['online_amount'], 0.01);
        $this->assertEqualsWithDelta(200.0, $split['cash_amount'], 0.01);

        $holder = $this->service()->resolveHolderForDelivery($order);

        $this->assertNotNull($holder);
        $this->assertSame(\App\Enums\LiabilityHolderType::Courier, $holder[0]);
