...
class ProcessingModuleSeeder extends Seeder
{
    ...
    private function seedWarehouseAndGl(): void
    {
        ...
        $stock = DB::table('stocks')->where('warehouse_type', 'materials_at_vendor')->first()
            ?? DB::table('stocks')->where('name', ProcessingWarehouseResolver::WAREHOUSE_NAME)->first()
            ?? DB::table('stocks')->whereIn('name', ProcessingWarehouseResolver::LEGACY_WAREHOUSE_NAMES)->first();
