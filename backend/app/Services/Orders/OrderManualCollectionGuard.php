...
final class OrderManualCollectionGuard
{
    ...
    /**
     * هل يُغلق الطلب بـ «تم التحصيل»؟
     *
     * لا يُعتبر التحصيل كاملاً إذا بقيت ذمة شركة تحصيل، أو سطور شحن/تحصيل مفتوحة،
     * أو مبلغ مستحق شحن مخزَّن — حتى لا يحوّل التحصيل الجزئي الطلب إلى «تم التحصيل».
     */
    public function shouldMarkOrderFullyCollected(Order $order): bool
    {
        ...
        if ($this->openShippingDetailRows($order)->isNotEmpty()) {
            return false;
        }

        $shippingReceivable = $order->order_details?->shipping_receivable_amount;
        if ($shippingReceivable !== null && round((float) $shippingReceivable, 2) > 0.009) {
            return false;
        }
