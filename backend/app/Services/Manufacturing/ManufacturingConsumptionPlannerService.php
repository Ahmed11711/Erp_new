...
final class ManufacturingConsumptionPlannerService
{
    ...
    public function preview(int $outputProductId, float $batchQty, ?array $overrides = null): array
    {
        ...
        $overrides = $this->discardStaleUnitQuantityOverrides($lines, $overrides);
    ...
    /**
     * Ignore leftover per-unit BOM quantities after the batch size changed.
     * Real substitutions / edited quantities are kept.
     *
     * @param  list<array<string, mixed>>  $lines
     * @param  list<array{bom_item_id?:int, resolved_category_id?:int, quantity?:float|int|string}>|null  $overrides
     * @return list<array{bom_item_id?:int, resolved_category_id?:int, quantity?:float|int|string}>|null
     */
    private function discardStaleUnitQuantityOverrides(array $lines, ?array $overrides): ?array
    {
        if ($overrides === null || $overrides === []) {
            return $overrides;
        }
        ...
        $byBom = [];
        foreach ($lines as $line) {
            $byBom[(int) $line['bom_item_id']] = $line;
        }
        ...
        foreach ($overrides as $override) {
            ...
            if ($bomId <= 0 || ! isset($byBom[$bomId])) {
                continue;
            }

            $line = $byBom[$bomId];
            $qty = (float) ($override['quantity'] ?? 0);
            $bomUnit = (float) ($line['bom_unit_qty'] ?? 0);
            $defaultQty = (float) ($line['default_quantity'] ?? 0);
            $defaultCat = (int) ($line['default_resolved_category_id'] ?? $line['resolved_category_id']);
            $catId = (int) ($override['resolved_category_id'] ?? 0);

            if ($catId > 0 && $catId !== $defaultCat) {
                $realCustom++;
                continue;
            }

            if (abs($qty - $defaultQty) <= 0.000001) {
                continue;
            }

            if ($bomUnit > 0 && abs($qty - $bomUnit) <= 0.000001 && abs($defaultQty - $bomUnit) > 0.000001) {
                $staleMatches++;
                continue;
            }
        ...
        if ($realCustom === 0 && $staleMatches > 0) {
            return null;
        }
