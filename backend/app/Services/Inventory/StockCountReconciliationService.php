...
class StockCountReconciliationService
{
        ...
        foreach ($allSheetRows as $row) {
            ...
            if (!$match['category']) {
                if ($unitCost !== null && (float) $unitCost > 0) {
                    $rowWarnings[] = 'صنف جديد — سيتم إنشاؤه بسعر ' . round((float) $unitCost, 4);
                } else {
                    $rowWarnings[] = 'صنف جديد — سيتم إنشاؤه بسعر 0 (لا يوجد سعر في عمود السعر)';
                }
            }
            if ($diff !== null && $diff < 0) {
                ...
                try {
                    if ($row->is_new_product) {
                        ...
                        if (abs($row->excel_quantity) > 0.000001) {
                            $excelCost = $row->excel_unit_cost !== null ? (float) $row->excel_unit_cost : null;
                            $this->applyAdjustment($cat, (float) $row->excel_quantity, 0, $userId, $excelCost);
                            $adjusted++;
                        }
                    } else {
    ...
    private function createNewProduct(InventoryCountImportRow $row, int $defaultProductionId): Category
    {
        ...
        $price = $row->excel_unit_cost !== null ? (float) $row->excel_unit_cost : 0.0;
    ...
    private function applyAdjustment(Category $cat, float $diff, float $systemQty, ?int $userId, ?float $unitCostOverride = null): void
    {
        $avg = CategoryInventoryCostService::averageCostForCategoryIssue((int) $cat->id);
        if ($avg <= 0 && $unitCostOverride !== null && $unitCostOverride > 0) {
            $avg = $unitCostOverride;
        }
    ...
    private function mapWarehouseName(string $name): string
    {
        ...
        // Keyword mapping
        $patterns = [
            'مستلزمات تشغيل وأدوات تشغيل' => ['مستلزم', 'أدوات تشغيل', 'operating supply', 'supplies'],
            'مخزن منتج تام' => ['تام', 'نهائي', 'جاهز', 'finished', 'منتج تام'],
            'مخزن منتج تحت التشغيل' => ['تحت التشغيل', 'wip', 'شبه', 'نصف'],
    ...
    private function detectWarehouseFromTitle(string $title): ?string
    {
        ...
        $map = [
            'مستلزم' => 'مستلزمات تشغيل وأدوات تشغيل',
            'أدوات تشغيل' => 'مستلزمات تشغيل وأدوات تشغيل',
            'خام' => 'مخزن مواد خام',
            'raw' => 'مخزن مواد خام',
            'تحت التشغيل' => 'مخزن منتج تحت التشغيل',
