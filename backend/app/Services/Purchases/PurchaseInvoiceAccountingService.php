...
class PurchaseInvoiceAccountingService
{
    ...
    public function applyProductLines(Purchase $purchase, array $products, PurchaseInvoiceKind $kind): array
    {
        ...
        foreach ($products as $product) {
            ...
            if (! $catId) {
                throw new \RuntimeException('تعذر ربط الصنف بالمخزن (مواد خام / مستلزمات تشغيل): '.$product['product_name']);
            }
            ...
            $productionId = null;
            if (is_array($product) && ! empty($product['production_id'])) {
                $productionId = (int) $product['production_id'];
            }
            if ($productionId && $productionId > 0) {
                Category::query()->whereKey($catId)->update(['production_id' => $productionId]);
            }
