...
class OfferConvertToOrderService
{
    ...
    public function convert(Offers $offer, array $payload): array
    {
        ...
        $productsTotal = round(array_sum(array_map(
            static fn ($line) => (float) ($line['total'] ?? 0),
            $details
        )), 3);
        $totalInvoice = round((float) ($payload['total_invoice'] ?? $productsTotal), 3);
        ...
        $requestedVat = (float) ($payload['vat'] ?? $offer->vat ?? 0);
        $vat = VatCalculator::amount($productsTotal, $requestedVat > 0);
        $netTotal = round($totalInvoice + $shippingCost + $vat - $discount - $prepaid, 3);
