...
class InventoryGlPostingService
{
    ...
    /**
     * صرف مستلزمات تشغيل للإنتاج:
     * مدين مصروفات صناعية غير مباشرة - مستلزمات تشغيل / دائن مخزون مستلزمات التشغيل.
     */
    public function postOperatingSuppliesIssue(float $amount, string $description, ?int $userId = null): void
    {
        if ($amount <= 0.00001) {
            return;
        }

        $overheadId = \App\Services\Inventory\OperatingSuppliesWarehouseResolver::ensureOverheadAccountId();
        $inventoryId = \App\Services\Inventory\OperatingSuppliesWarehouseResolver::ensureInventoryAccountId();
        ...
        $this->postTwoLineDailyEntry(
            ...
            0,
            $amount,
            'مصروفات صناعية غير مباشرة — مستلزمات تشغيل',
            'خصم من مخزون مستلزمات التشغيل',
            $userId
        );
        $this->accountingService->updateAccountHierarchyBalances($overheadId);
        $this->accountingService->updateAccountHierarchyBalances($inventoryId);
    }

    /**
     * جرد فعلي — نقص: مدين مصروف عجز / دائن مخزون.
     */
    public function postPhysicalCountLoss(float $amount, TreeAccount $inventoryAccount, string $description, ?int $userId = null, ?DateTimeInterface $entryDate = null): ?DailyEntry
    {
        if ($amount <= 0.00001) {
            return null;
        }
        ...
        $entry = $this->postTwoLineDailyEntry(
            $description,
            $loss->id,
            $amount,
            0,
     ...
     */
    public function postPhysicalCountGain(float $amount, TreeAccount $inventoryAccount, string $description, ?int $userId = null, ?DateTimeInterface $entryDate = null): ?DailyEntry
    {
        if ($amount <= 0.00001) {
            return null;
        }
     ...
     */
    public function postSalesReturnInventoryRestoreByWarehouse(
        ...
        if ($orderId) {
            ...
            foreach ($clean as $accId => $amt) {
                $journalLines[] = [
                    ...
                    'credit' => 0.0,
                    'description' => 'إرجاع مخزون — مرتجع مبيعات',
                ];
            }
            $journalLines[] = [
                'account_id' => (int) $cogs->id,
                'debit' => 0.0,
                'credit' => round($sumDr, 2),
            ...
            app(LedgerJournalService::class)->postBalancedJournal(
                ...
                $description,
                $orderId,
                'RETURN-COGS-'.$orderId.'-'.now()->format('YmdHis'),
                $userId,
                $date
            );
     ...
     */
    public function postCogsShipment(
        float $totalCogs,
        ...
        if ($orderId) {
            ...
            $batch = 'COGS-'.$orderId.'-'.now()->format('YmdHis');
            app(LedgerJournalService::class)->postBalancedJournal(
                $lines,
                $description,
                $orderId,
