...
class ShopifyOrderImportService
{
    ...
    private function normalizePhone(string $phone): string
    {
        ...
        // يشمل صيغة واجهة شوبيفاي مثل: +20 10 01866665
        ...
        // 20 + رقم محلي يبدأ بـ 0 (13 رقم): 2001001866665 → 01001866665
        if (str_starts_with($digits, '20') && strlen($digits) === 13 && ($digits[2] ?? '') === '0') {
            return substr($digits, 2);
        }
    ...
    /**
     * جودة الرقم لاختيار الأفضل عند تعارض المصادر.
     * أرقام الموبايل المصرية 01[0125]xxxxxxxx لها أولوية أعلى.
     */
    private function phoneQualityScore(string $normalized): int
    {
        if ($this->isValidEgyptianMobile($normalized)) {
            return 3;
        }
    ...
    private function isValidEgyptianMobile(string $normalized): bool
    {
        return preg_match('/^01[0125]\d{8}$/', $normalized) === 1;
    }
    ...
    /**
     * اختيار الهاتف من شوبيفاي مع تفضيل هاتف عنوان الشحن (ما يظهر في صفحة الأوردر)
     * ثم بيانات الطلب الحالي، قبل هاتف ملف العميل الذي قد يكون قديماً.
     *
    ...
    {
        $candidates = [
            // 1) هاتف عنوان الشحن في الأوردر — المصدر الأساسي الظاهر في Shopify
        ...
        // 3) حقول فورم الطلب الحالي (note_attributes) قبل ملف العميل
        ...
        // 4) بيانات العميل/الفاتورة كاحتياطي أخير (قد تكون من طلبات سابقة)
        ...
        foreach ($candidates as $index => $candidate) {
            ...
            // الجودة أولاً، وعند التساوي يبقى مصدر أعلى أولوية (index أصغر)
            $score = ($this->phoneQualityScore($normalized) * 1000) - $index;
            if ($score > $bestScore) {
                $bestScore = $score;
