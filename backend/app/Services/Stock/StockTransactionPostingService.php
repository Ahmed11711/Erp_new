...
class StockTransactionPostingService
{
    ...
    private function applyLine(string $code, StockTransaction $doc, StockTransactionItem $item): void
    {
        ...
        switch ($code) {
            case 'STOCK_OUT':
                ...
                $lineTotal = $total > 0 ? $total : ($qty * $unit);
                ...
                $this->maybePostOperatingSuppliesIssueGl($category, $lineTotal, $doc);
                break;
    ...
    private function maybePostOperatingSuppliesIssueGl(Category $category, float $amount, StockTransaction $doc): void
    {
        if ($amount <= 0.00001) {
            return;
        }
        ...
        $stock = null;
        if ($doc->warehouse_id) {
            $stock = Stock::query()->find((int) $doc->warehouse_id);
        }
        if (! $stock && $category->stock_id) {
            $stock = Stock::query()->find((int) $category->stock_id);
        }

        if (! OperatingSuppliesWarehouseResolver::isOperatingSuppliesWarehouse($stock, (string) ($category->warehouse ?? ''))) {
            return;
        }

        $label = 'صرف مستلزمات تشغيل — '.($category->category_name ?? ('#'.$category->id));
        if ($doc->notes) {
            $label .= ' ('.$doc->notes.')';
        }

        app(InventoryGlPostingService::class)->postOperatingSuppliesIssue(
            $amount,
            $label,
            auth()->id()
        );
