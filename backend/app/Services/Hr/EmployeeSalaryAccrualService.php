...
class EmployeeSalaryAccrualService
{
     ...
     *
     * @param  bool  $requirePayableAccount  false = حفظ مبلغ المستحق بدون قيد محاسبي إن لم يوجد ربط حساب
     * @return array{amount:float, accrual:EmployeeMonthAccrual|null, skipped:bool, reason?:string}
     */
    public function accrueForMonth(
        int $employeeId,
        int $month,
        int $year,
        ?float $overrideAmount = null,
        bool $requirePayableAccount = true
    ): array {
        ...
        if (! $payableAccount && $requirePayableAccount) {
        ...
        // بدون حساب مربوط: تحديث مبلغ المستحق فقط (بدون قيد)
        if (! $payableAccount) {
            if ($existing) {
