...
class EmployeeFingerPrintSheetNotificationService
{
    public function notifyAdmins(EmployeeFingerPrintSheet $sheet, string $actionLabel): void
    {
        ...
        $this->notifyAdminsAboutEmployee((int) $sheet->employee_id, $note);
    }

    public function notifyAdminsAboutEmployee(?int $employeeId, string $note): void
    {
        $actor = Auth::user();
        ...
        foreach ($admins as $admin) {
            Notification::create([
                ...
                'send_to' => $admin->id,
                'type' => 'كشف حضور',
                'ref' => $employeeId ? (string) $employeeId : null,
                'order_id' => null,
                'note' => $note,
            ]);
