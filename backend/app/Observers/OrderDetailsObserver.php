...
class OrderDetailsObserver
{
    public function saved(OrderDetails $orderDetails): void
    {
        ...
        DB::afterCommit(function () use ($orderId) {
            ...
            $salesAccounting = app(SalesOrderAccountingService::class);
            if ($salesAccounting->shouldSkipInvoiceRecognition($order)) {
                return;
            }
            try {
                $salesAccounting->refreshOrderRecognition($order, true);
            } catch (\Throwable $e) {
