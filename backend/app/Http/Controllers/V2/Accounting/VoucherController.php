...
class VoucherController extends Controller
{
    ...
    private function applyCollectionCompanyOrderSettlement(Voucher $voucher, $orderRows, float $voucherAmount): void
    {
        ...
        foreach ($orderRows->sortBy('order_id') as $row) {
            ...
            $fullyCollected = $newReceivable <= 0.009
                && app(\App\Services\Orders\OrderManualCollectionGuard::class)
                    ->shouldMarkOrderFullyCollected($order);

            $od->collection_status = $fullyCollected
                ? OrderCollectionStatus::Collected->value
                : OrderCollectionStatus::Partial->value;
            $od->save();

            // لا تُغلق حالة الطلب إلى «تم التحصيل» عند تحصيل جزئي أو بقاء ذمة شحن.
            if ($fullyCollected && $order->order_status !== 'تم التحصيل') {
