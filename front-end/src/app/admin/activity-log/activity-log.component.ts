...
export class ActivityLogComponent implements OnInit {
  ...
  openDetails(log: any, event?: Event): void {
    event?.preventDefault();
    event?.stopPropagation();
    this.selectedLog = log || null;
  }
  ...
  hasDetails(log: any): boolean {
    ...
    return description !== '' || this.hasMeta(log?.meta);
  }

  hasMeta(meta: any): boolean {
    if (meta == null || meta === '') {
      return false;
    }
  ...
  metaEntries(value: any): Array<{ key: string; value: any }> {
    if (value == null) {
      return [];
    }
