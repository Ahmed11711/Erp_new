...
export class WhatsAppService {
  ...
  setCustomerArchive(customerId: number, archived: boolean): Observable<any> {
    return this.http.patch<any>(
      `${environment.Url}/whatsapp/customers/${customerId}/archive`,
      { archived },
      { headers: WHATSAPP_SKIP_GLOBAL_LOADING }
    );
  }

  archiveAllCustomers(params?: { from_date?: string; to_date?: string }): Observable<any> {
    return this.http.post<any>(
      `${environment.Url}/whatsapp/customers/archive-all`,
      params || {},
      { headers: WHATSAPP_SKIP_GLOBAL_LOADING }
    );
  }
