...
export class OrderFulfillmentPanelComponent implements OnChanges {
  ...
  load(): void {
    ...
    this.orderService.getOrderFulfillment(this.orderId).subscribe({
      ...
      error: () => {
        this.loading = false;
        this.loadError = 'تعذر تحميل حالة التسليم والتحصيل.';
        this.cdr.detectChanges();
      },
    });
  }

  get isMixedOnlineCash(): boolean {
    return !!this.data?.financial?.is_mixed_online_cash && !!this.data?.financial?.mixed_split;
  }

  get mixedSplit(): any {
    return this.data?.financial?.mixed_split || {};
  }
  ...
  async transferLiability(): Promise<void> {
    ...
    const shipOpts = shippingList
      ...
      .join('');
    const collOpts = this.isMixedOnlineCash
      ? ''
      : collectionList
    ...
    if (!shipOpts && !collOpts) {
      Swal.fire('تنبيه', this.isMixedOnlineCash
        ? 'حدد شركة شحن أو مندوب أولاً لنقل ذمة الكاش.'
        : 'لا توجد شركات شحن أو تحصيل متاحة.', 'warning');
      return;
    }
    ...
    const amountLine =
      amount != null ? `<p style="margin:0 0 8px">مبلغ الكاش: <b>${this.escapeHtml(amount)}</b></p>` : '';
    const currentLine = providerName
      ...
      : '';
    const mixedNote = this.isMixedOnlineCash
      ? `<p style="margin:0 0 8px;color:#9a3412">الأونلاين يفضل على شركة التحصيل. هنا تُنقل ذمة الكاش فقط للمندوب / شركة الشحن.</p>`
    ...
    const result = await Swal.fire<{ to_holder_type: string; to_holder_id: number }>({
      title: this.isMixedOnlineCash ? 'نقل ذمة الكاش' : 'نقل الذمة',
      html: `
