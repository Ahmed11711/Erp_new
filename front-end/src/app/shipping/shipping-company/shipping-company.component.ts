...
export class ShippingCompanyComponent implements OnInit {
  ...
  /** كل الصفوف من الـ API قبل التصفية */
  allData: any[] = [];
  searchQuery = '';
  ...
  private applyFilters(): void {
    ...
    if (this.showUnlinkedOnly) {
      rows = rows.filter((item: any) => !this.isLinked(item));
    }
    const q = this.searchQuery.trim().toLowerCase();
    if (q) {
      rows = rows.filter((item: any) => this.matchesSearch(item, q));
    }
    this.data = rows;
  }

  private matchesSearch(item: any, q: string): boolean {
    const name = String(item?.name || '').toLowerCase();
    const type = String(item?.type || '').toLowerCase();
    const acc = item?.receivable_tree_account;
    const code = String(acc?.code ?? item?.receivable_tree_account_id ?? '').toLowerCase();
    const accName = String(acc?.name || '').toLowerCase();
    return name.includes(q) || type.includes(q) || code.includes(q) || accName.includes(q);
  }
