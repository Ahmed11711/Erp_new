...
export class ShipOrderComponent implements OnInit {
  ...
  recalcSplit(): void {
    ...
    const net = parseFloat(this.orderSnap.net_total) || 0;
    const prepaid = parseFloat(this.orderSnap.prepaid_amount) || 0;
    const prepaidExceedsNet = prepaid > net + 0.02;
    const grandBasis = prepaidExceedsNet ? Math.round((net + prepaid) * 100) / 100 : Math.round(net * 100) / 100;
    ...
    if (this.manualSplitMode && this.manualShippingAmount != null && this.manualCollectionAmount != null) {
      ...
      if (Math.abs(sum - roundedNet) > 0.02 && Math.abs(sum - grandBasis) > 0.02) {
        this.splitError = prepaidExceedsNet
          ? `المجموع (${sum}) ≠ الصافي (${roundedNet}) ولا الإجمالي (${grandBasis})`
          : `المجموع (${sum}) ≠ صافي الطلب (${roundedNet})`;
      } else {
        this.splitError = '';
      }
    } else if (this.showCollectionSplit && prepaid > 0) {
      if (prepaidExceedsNet) {
        this.computedCollectionAmount = Math.round(prepaid * 100) / 100;
