...
export class OrderDetailsComponent implements OnInit, OnDestroy{
  ...
  pageLoading = false;
  pageError = '';
  private routeSubs = new Subscription();
  ...
  accountingCycleLoading = false;
  accountingCycleError = '';
  accountingCycleOpen = false;

  ngOnInit(): void {
    this.routeSubs.add(this.route.paramMap.subscribe((params) => {
      const nextId = Number(params.get('id'));
      this.id = Number.isFinite(nextId) && nextId > 0 ? nextId : 0;
      if (this.id) {
        this.getOrder();
      }
    }));

    this.routeSubs.add(this.route.queryParams.subscribe((qp: any) => {
  ...
  getOrder(){
    ...
    this.orderService.getOrderById(this.id).subscribe({
      next: (result: any) => {
        this.applyOrderPayload(result);
        this.pageLoading = false;
        this.cdr.detectChanges();
      },
      error: (err) => {
        this.pageLoading = false;
        this.pageError = err?.error?.message || 'تعذر تحميل بيانات الطلب. أعد المحاولة.';
        this.cdr.detectChanges();
      },
    });
  }

  private applyOrderPayload(result: any): void {
    this.order = result;
    this.orderType = result?.order_type;

    this.tempReviewNotification = result?.tempReviewNotification || [];

    this.order_products = Array.isArray(result?.order_products) ? result.order_products : [];
    ...
    this.reviewd = this.order?.order_details?.reviewed == 1;
    this.reviewdNote = this.order?.order_details?.reviewed_note || '';
    this.userReviewd = this.order?.order_details?.user_reviewed == 1;
    ...
    this.loadAccountingCycle();
  }

  toggleAccountingCycle(): void {
    this.accountingCycleOpen = !this.accountingCycleOpen;
  }

  visibleAccountingStages(): any[] {
    return (this.accountingCycle?.stages || []).filter((stage: any) =>
      stage?.applicable || stage?.posted || (Array.isArray(stage?.journals) && stage.journals.length)
    );
  }

  otherAccountingJournals(): any[] {
    return (this.accountingCycle?.journals || []).filter((journal: any) =>
      !journal?.stage || journal.stage === 'other'
    );
  }

  loadAccountingCycle(event?: Event): void {
    event?.stopPropagation();
    if (!this.isDetails || !this.id) {
      return;
    }
    this.accountingCycleLoading = true;
    this.accountingCycleError = '';
    this.orderService.getAccountingCycle(this.id).subscribe({
      next: (res) => {
        this.accountingCycle = res;
        this.accountingCycleLoading = false;
        this.cdr.markForCheck();
      },
      error: () => {
        this.accountingCycle = null;
        this.accountingCycleLoading = false;
        this.accountingCycleError = 'تعذر تحميل القيود المحاسبية للطلب.';
        this.cdr.markForCheck();
      },
    });
  }
