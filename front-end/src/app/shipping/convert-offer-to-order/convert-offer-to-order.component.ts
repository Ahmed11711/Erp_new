...
import { applyOfferVat, calculateVat } from 'src/app/shared/utils/vat';
...
export class ConvertOfferToOrderComponent implements OnInit {
  ...
  private syncVatFromProducts(): void {
    if ((Number(this.form.value.vat) || 0) <= 0 && (Number(this.offer?.vat) || 0) <= 0) {
      return;
    }
    this.form.patchValue({ vat: calculateVat(this.productsTotal) }, { emitEvent: false });
  }
  ...
  submit() {
    ...
    if (this.unmatchedCount > 0) {
      const names = this.lines
        ...
        .map((l) => l.offer_name)
        .filter((n) => !!n);
      const list = names.length ? `<br><b>${names.join('، ')}</b>` : '';
      Swal.fire({
        title: 'أصناف غير موجودة',
        html: `بعض المنتجات غير موجودة في النظام.${list}<br>هل تريد ربطها في الطلب الآن أم لاحقاً؟`,
        icon: 'question',
        showDenyButton: true,
        showCancelButton: true,
        ...
        if (choice.isDenied) {
          const offerType = this.offer?.offer === 'offer2' ? 'offer2' : 'offer1';
          this.router.navigate(['/dashboard/permissions', offerType, this.offerId], {
            queryParams: { match: '1' },
          });
          return;
        }
  ...
  private createMissingThenConvert() {
    this.saving = true;
    this.offerService.createMissingCategories(this.offerId).subscribe({
      next: (res: any) => {
        const analysis = res?.analysis?.lines || [];
        this.lines = this.lines.map((line) => {
          const gap = analysis.find((g: any) => g.offer_line_id === line.offer_line_id);
          const catId = gap?.category?.id || line.category_id;
          return {
            ...line,
            category_id: catId || null,
            category_name: gap?.category?.category_name || line.category_name,
            status: gap?.status || line.status,
          };
        });
        this.saving = false;
        if (this.unmatchedCount > 0) {
          Swal.fire(
            'تنبيه',
            'ما زالت هناك أصناف غير مربوطة. يمكنك مطابقتها يدوياً لاحقاً من تفاصيل العرض.',
            'warning'
          );
          return;
        }
      ...
      error: (err) => {
        this.saving = false;
        Swal.fire('خطأ', err?.error?.message || 'فشل إنشاء الأصناف الناقصة', 'error');
      },
    });
  }
}
