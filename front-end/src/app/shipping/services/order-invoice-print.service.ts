...
export interface OrderInvoicePrintOptions {
  ...
  autoPrint?: boolean;
  /** Reload opener after print is triggered. */
  reloadAfterPrint?: boolean;
}
...
export class OrderInvoicePrintService {
  /**
   * Opens a native HTML print window (fast, works for large batches).
   * Uses a blob URL + zero @page margins so Chrome does not inject the app page URL.
   */
  openPrintWindow(orders: any[], options: OrderInvoicePrintOptions = {}): boolean {
    ...
    // Blob URL avoids printing the dashboard/shipping_list URL in browser headers.
    ...
    if (autoPrint) {
      void this.triggerPrintWhenReady(printWindow, objectUrl, options.reloadAfterPrint === true);
    } else {
  ...
  /** Save invoice(s) as a PDF file download (best for small/medium batches). */
  async downloadPdf(orders: any[], options: OrderInvoicePrintOptions = {}): Promise<void> {
    ...
    const blob = await this.buildPdfBlob(orders, options);
  ...
  private async triggerPrintWhenReady(
    printWindow: Window,
    objectUrl: string,
    reloadAfterPrint: boolean
  ): Promise<void> {
  ...
  private async buildPdfBlob(
    orders: any[],
    options: OrderInvoicePrintOptions
  ): Promise<Blob> {
    const showInvoiceDate = options.showInvoiceDate !== false;
    const size = options.size || 'A4';
    ...
    host.innerHTML = `<style>${this.printStyles(size)}</style><div id="capture" dir="ltr">${pages}</div>`;
    ...
    try {
      await this.waitForImages(document, host);
...
<body dir="ltr">
  ...
  private waitForImages(doc: Document, root?: ParentNode): Promise<void> {
  ...
  private showPrintError(printWindow: Window): void {
    ...
    try {
      ...
      printWindow.document.write(
        '<p style="font-family:sans-serif;padding:2rem;margin:0;color:#b00020;direction:rtl;">تعذر تحضير الفاتورة للطباعة.</p>'
      );
  ...
  private printStyles(size: string): string {
    return `
      * {
      ...
      /* margin:0 removes Chrome's URL header/footer; body padding keeps content inset */
      @media print {
