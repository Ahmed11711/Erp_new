...
export class TreeAccountService {
  private pickerCache$: Observable<TreeAccountResponse> | null = null;
  ...
  /** قائمة خفيفة (id/code/name) للاختيار — بدون شاشة تحميل عامة، مع تخزين مؤقت. */
  getPickerList(): Observable<TreeAccountResponse> {
    if (!this.pickerCache$) {
      this.pickerCache$ = this.http.get<TreeAccountResponse>(`${environment.Url}/tree_accounts`, {
        params: { flat: '1' },
        headers: SKIP_GLOBAL_LOADING,
      }).pipe(shareReplay(1));
    }
