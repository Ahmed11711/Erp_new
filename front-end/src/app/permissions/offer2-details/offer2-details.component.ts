...
export class Offer2DetailsComponent implements OnInit {
  offer: any = {};
  categories: any[] = [];
  imgUrl!: string;
  ...
  showLinkLeadPanel = false;
  showProductGapsPanel = false;
  deleting = false;
  leads: any[] = [];
  selectedLead: any = null;
  linkingLead = false;
  leadSearchKeyword = 'company_name';

  canEditOffer(): boolean {
    if (this.offer?.debt_posted_at && !this.offer?.converted_order_id) {
      return false;
    }
  ...
  canDeleteOffer(): boolean {
    if (this.offer?.converted_order_id || this.offer?.debt_posted_at) {
      return false;
    }
    return this.rbac.can('offers.delete') || this.rbac.can('system.rbac');
  }

  deleteOffer(): void {
    if (!this.offer?.id || !this.canDeleteOffer() || this.deleting) {
      return;
    }
    Swal.fire({
      title: 'حذف عرض السعر؟',
      text: `سيتم حذف العرض #${this.offer.id} وجميع أصنافه نهائياً.`,
      icon: 'warning',
      ...
      this.offerService.deleteOffer(this.offer.id).subscribe({
        next: () => {
          this.deleting = false;
          Swal.fire({ icon: 'success', title: 'تم الحذف', timer: 1400, showConfirmButton: false });
          this.router.navigate(['/dashboard/permissions/priceoffer']);
        },
        error: (err) => {
          this.deleting = false;
          Swal.fire('خطأ', err?.error?.message || 'تعذر حذف عرض السعر', 'error');
        },
      });
    });
  }
  ...
  toggleLinkLeadPanel(): void {
    this.showLinkLeadPanel = !this.showLinkLeadPanel;
    if (this.showLinkLeadPanel && !this.leads.length) {
      this.loadLeads();
    }
  }

  loadLeads(q = ''): void {
    this.leadsService.getLeads({ itemsPerPage: 50, page: 1, q }).subscribe({
      next: (res: any) => {
  ...
  get isLeadLinked(): boolean {
    return !!this.offer?.corporate_sales_lead_id;
  }

  linkToLead(): void {
    const leadId = Number(this.selectedLead?.id);
    if (!leadId || !this.offer?.id || this.linkingLead) {
      Swal.fire('تنبيه', 'اختر Lead أولاً', 'warning');
      return;
    }
    this.linkingLead = true;
    this.offerService.linkLead(this.offer.id, leadId).subscribe({
      next: (res: any) => {
        this.linkingLead = false;
        this.offer = res.offer || this.offer;
        this.selectedLead = null;
        Swal.fire({ icon: 'success', title: 'تم الربط بالـ Lead', timer: 1400, showConfirmButton: false });
      },
      error: (err) => {
        this.linkingLead = false;
        Swal.fire('خطأ', err?.error?.message || 'تعذر ربط العرض بالـ Lead', 'error');
      },
    });
  }
  ...
  loadOffer(id: any) {
    this.offerService.getOfferById(id).subscribe((res: any) => {
      ...
      if (this.route.snapshot.queryParamMap.get('match') === '1') {
        this.showProductGapsPanel = true;
      }
    });
  }
  ...
  downloadPDF() {
    ...
    this.quotationExport.openPrintPreview(
      element,
      this.offer.id,
      this.offer?.customer_company?.name || this.offer?.quote || this.offer?.contact_person,
      this.offer?.dateFrom || this.offer?.dateTo
    );
  }

  /** Direct file download named after the client + date. */
  saveQuotationPdf() {
    const element = document.getElementById('capture');
    ...
    this.quotationExport.downloadPdf(
      element,
      this.offer.id,
      this.offer?.customer_company?.name || this.offer?.quote || this.offer?.contact_person,
      this.offer?.dateFrom || this.offer?.dateTo
