...
export class OfferService {
  ...
  getOffers(items: number, page: number, search?: {
    ...
    corporate_sales_lead_id?: number | string;
  }) {
    const params: any = {
      itemsPerPage: items,
      page,
    };
    const offerId = search?.id != null ? String(search.id).trim() : '';
    if (offerId !== '') {
      params.id = offerId;
    }
    ...
    if (search?.corporate_sales_lead_id) {
      params.corporate_sales_lead_id = String(search.corporate_sales_lead_id);
    }
  ...
  deleteOffer(id: number | string){
    return this.http.delete(`${environment.Url}/offer/${id}`);
  }

  linkLead(offerId: number | string, leadId: number){
    return this.http.post(`${environment.Url}/offer/${offerId}/link-lead`, {
  ...
  syncConvertedOrder(offerId: number | string, createMissingCategories = false) {
    return this.http.post(`${environment.Url}/offer/${offerId}/sync-converted-order`, {
