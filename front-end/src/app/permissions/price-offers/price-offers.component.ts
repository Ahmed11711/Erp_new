...
export class PriceOffersComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.route.queryParamMap.pipe(takeUntil(this.destroy$)).subscribe((params) => {
      ...
      this.filterLeadId = leadId > 0 ? leadId : null;
      this.filterLeadLabel = (params.get('lead_name') || '').trim();
      this.page = 0;
      this.getOffers();
    });
  }
  ...
  getOffers() {
    const pageSize = Number(this.pageSize) > 0 ? Number(this.pageSize) : 15;
    const page = Number(this.page) >= 0 ? Number(this.page) : 0;
    this.pageSize = pageSize;
    this.page = page;
    this.loading = true;

    this.offerService.getOffers(pageSize, page + 1, {
      id: this.idSearch,
      ...
      corporate_sales_lead_id: this.filterLeadId || undefined,
    }).subscribe({
      next: (res: any) => {
        this.offers = Array.isArray(res?.data) ? res.data : [];
        this.length = Number(res?.total) || 0;
        const perPage = Number(res?.per_page);
        if (perPage > 0) {
          this.pageSize = perPage;
        }
        // Laravel may clamp page; keep paginator in sync with current_page (1-based).
        const currentPage = Number(res?.current_page);
        if (currentPage > 0) {
  ...
  onPageChange(event: { pageIndex: number; pageSize: number }) {
    ...
    const nextSize = Number(event?.pageSize) > 0 ? Number(event.pageSize) : this.pageSize;
    const nextPage = Number(event?.pageIndex) >= 0 ? Number(event.pageIndex) : 0;
    if (nextSize === this.pageSize && nextPage === this.page) {
      return;
    }
  ...
  canEditOffer(elm?: any): boolean {
    if (elm?.debt_posted_at && !elm?.converted_order_id) {
      return false;
    }
    return this.rbac.can('offers.edit') || this.rbac.can('system.rbac');
  }

  canDeleteOffer(elm?: any): boolean {
    if (elm?.converted_order_id || elm?.debt_posted_at) {
      return false;
    }
    return this.rbac.can('offers.delete') || this.rbac.can('system.rbac');
  }

  deleteOffer(elm: any): void {
    if (!this.canDeleteOffer(elm) || this.deletingId != null) {
      return;
    }
    ...
    Swal.fire({
      title: 'حذف عرض السعر؟',
      text: `سيتم حذف العرض #${id} وجميع أصنافه نهائياً.`,
      icon: 'warning',
