...
export class CategoryService {
 ...
 nextItemCode(warehouse: string) {
  return this.http.get<{ item_code: string; prefix: string }>(`${environment.Url}/categories/next-item-code`, {
   params: { warehouse }
  });
 }
 ...
 changeCategoryQuantity(id, status, quantity, movementDate?: string) {
  let url = `${environment.Url}/categoryquantity?id=${id}&status=${status}&quantity=${quantity}`;
  if (movementDate) {
   url += `&movement_date=${encodeURIComponent(movementDate)}`;
  }
  return this.http.get(url);
 }

 /** صرف مستلزمات تشغيل لقسم إنتاج (خصم مخزون + قيد مصروفات صناعية غير مباشرة). */
 issueOperatingSupplies(payload: {
  category_id: number;
  qty: number;
  ...
  return this.http.post(`${environment.Url}/inventory/operating-supplies/issue`, payload);
 }

 ensureOperatingSuppliesWarehouse() {
  return this.http.get(`${environment.Url}/inventory/operating-supplies/ensure`);
 }
 ...
 categoriesStatusReports(items: number, page: number, search: any) {
  return this.http.get(`${environment.Url}/reports/categoriesStatusReports?itemsPerPage=${items}&page=${page}`, { params: search })
 }

 // ✅ الدالة الجديدة لتحديث الرصيد مباشرة
 updateQuantity(id: number, quantity: number, movementDate?: string) {
  const body: { quantity: number; movement_date?: string } = { quantity };
  if (movementDate) {
   body.movement_date = movementDate;
  }
  return this.http.patch(`${environment.Url}/categories/${id}/quantity`, body);
 }
 ...
 previewAssignItemCodes(payload: { category_ids?: number[]; warehouse?: string }) {
  return this.http.post(`${environment.Url}/categories/assign-item-codes-preview`, payload);
 }

 assignItemCodes(payload: {
  category_ids?: number[];
  warehouse?: string;
  confirm_phrase: string;
  replace_existing?: boolean;
 }) {
  return this.http.post(`${environment.Url}/categories/assign-item-codes`, payload);
 }
