...
export class ListCategoriesComponent implements OnInit, OnDestroy {
 ...
 get canAssignItemCodes(): boolean {
  return this.user === 'Admin' || this.rbac.can('categories.assign_item_codes');
 }

 /** توليد أكواد الأصناف الفارغة دفعة واحدة — Admin أو صلاحية categories.assign_item_codes. */
 assignMissingItemCodes() {
  if (!this.canAssignItemCodes) {
   return;
  }

  const warehouse = String(this.warehouse || this.ware || '').trim();
  const selectedIds = Array.from(this.selectedItems.keys()).map((id) => Number(id)).filter((id) => id > 0);
  const payload: { category_ids?: number[]; warehouse?: string } = {};
  let scopeLabel = 'كل الأصناف في كل المخازن';

  if (selectedIds.length > 0) {
   payload.category_ids = selectedIds;
   scopeLabel = `الأصناف المحددة (${selectedIds.length})`;
  } else if (warehouse) {
   payload.warehouse = warehouse;
   scopeLabel = `أصناف «${warehouse}»`;
  }

  this.assigningItemCodes = true;
  Swal.fire({
   title: 'جاري مراجعة أكواد الأصناف…',
   allowOutsideClick: false,
   didOpen: () => Swal.showLoading(),
  });

  this.category.previewAssignItemCodes(payload).subscribe({
   next: (preview: any) => {
    this.assigningItemCodes = false;
    Swal.close();
    const missing = Number(preview?.missing ?? 0);
    const mismatched = Number(preview?.mismatched ?? 0);
    const matching = Number(preview?.matching ?? 0);
    if (missing === 0 && mismatched === 0) {
     Swal.fire({
      icon: 'info',
      title: 'الأكواد مكتملة',
      text: 'كل الأصناف في النطاق المحدد لها كود يتبع نمط المخزن بالفعل.',
     });
     return;
    }
    ...
    const rows: Array<{
     ...
     count: number;
     next_code: string;
    }> = Array.isArray(preview?.warehouses) ? preview.warehouses : [];
    const tableRows = rows
     .map((r) =>
      `<tr><td>${this.escHtml(String(r.warehouse ?? ''))}</td>` +
      `<td dir="ltr">${this.escHtml(String(r.prefix ?? ''))}</td>` +
      `<td>${Number(r.missing ?? 0)}</td>` +
      `<td>${Number(r.mismatched ?? 0)}</td>` +
      `<td dir="ltr">${this.escHtml(String(r.next_code ?? ''))}</td></tr>`
     )
     .join('');

    Swal.fire({
     title: 'توليد أكواد الأصناف',
     html:
      `<div dir="rtl" style="text-align:right;font-size:13px;">` +
      `<p class="mb-2">${this.escHtml(scopeLabel)}</p>` +
      `<p class="mb-2"><strong>بدون كود:</strong> ${missing} — ` +
      `<strong>كود لا يتبع النمط:</strong> ${mismatched} — ` +
      `<strong>تتبع النمط:</strong> ${matching}</p>` +
      `<p class="text-muted small mb-2">الخامات تبدأ بـ 10 (مثل 101001) وباقي المخازن ببادئة مختلفة. الأكواد المطابقة للنمط لن تتغير.</p>` +
      (tableRows
       ? `<table class="table table-sm table-bordered mb-2"><thead><tr>` +
         `<th>المخزن</th><th>البادئة</th><th>بدون كود</th><th>خارج النمط</th><th>أول كود</th>` +
         `</tr></thead><tbody>${tableRows}</tbody></table>`
       : '') +
      `<label class="d-flex align-items-start mt-2 mb-2" style="gap:8px;cursor:pointer;">` +
      `<input type="checkbox" id="replaceExistingCodes" ${mismatched > 0 && missing === 0 ? 'checked' : ''}>` +
      `<span>تعديل الأكواد السابقة أيضاً لتتبع نفس النمط (مثل ITM- أو أي كود يدوي). اتركه فارغاً لو هتولّد الفاضي فقط.</span>` +
      `</label>` +
      `<p class="mt-3 mb-1">للتأكيد اكتب: <strong>توليد الأكواد</strong></p>` +
      `</div>`,
     input: 'text',
     inputPlaceholder: 'توليد الأكواد',
     icon: 'question',
     showCancelButton: true,
     confirmButtonText: 'توليد الأكواد',
     cancelButtonText: 'إلغاء',
     width: '720px',
     preConfirm: () => {
      const phrase = String(Swal.getInput()?.value ?? '').trim();
      if (phrase !== 'توليد الأكواد') {
       Swal.showValidationMessage('اكتب «توليد الأكواد» بالضبط');
       return false;
      }
      const replaceExisting =
       (document.getElementById('replaceExistingCodes') as HTMLInputElement | null)?.checked === true;
      if (missing === 0 && !replaceExisting) {
       Swal.showValidationMessage('فعّل تعديل الأكواد السابقة، أو لا توجد أكواد فاضية للتوليد.');
       return false;
      }
      return { replace_existing: replaceExisting };
     },
    }).then((res: { isConfirmed: boolean; value?: { replace_existing?: boolean } | string }) => {
     if (!res.isConfirmed) {
      return;
     }
     const replaceExisting = typeof res.value === 'object'
      ? !!res.value?.replace_existing
      : false;
     this.assigningItemCodes = true;
     Swal.fire({
      title: 'جاري توليد الأكواد…',
      allowOutsideClick: false,
      didOpen: () => Swal.showLoading(),
     });
     this.category
      ...
      .subscribe({
       next: (out: any) => {
        this.assigningItemCodes = false;
        this.search();
        const assigned = Number(out?.result?.assigned ?? 0);
        const replaced = Number(out?.result?.replaced ?? 0);
        const failed = Number(out?.result?.failed ?? 0);
        Swal.fire({
         icon: failed > 0 ? 'warning' : 'success',
         title: out?.message ?? 'تم توليد الأكواد',
         text: failed > 0
          ? `تم توليد ${assigned} كود (منها ${replaced} تعديل لكود سابق)، وتعذر ${failed}.`
          : (replaced > 0
            ? `تم توليد ${assigned} كود، منها ${replaced} تعديل لكود سابق.`
            : `تم توليد ${assigned} كود.`),
         timer: 4500,
       ...
       error: (err) => {
        this.assigningItemCodes = false;
        const msg = err?.error?.message ?? err?.message ?? 'تعذر توليد الأكواد';
        Swal.fire({ icon: 'error', title: 'فشل التوليد', text: String(msg) });
       },
      });
    });
   },
   error: (err) => {
    this.assigningItemCodes = false;
    Swal.close();
    const msg = err?.error?.message ?? err?.message ?? 'تعذر معاينة أكواد الأصناف';
    Swal.fire({ icon: 'error', title: 'فشل المعاينة', text: String(msg) });
   },
  });
 }
 ...
 search() {
  ...
  this.searchSub?.unsubscribe();
  this.searchSub = this.category.searchCategories(this.pageSize, this.page + 1, this.param).subscribe({
   next: (data: any) => {
 ...
 updateQuantity(item: any, event: Event) {
  ...
  if (Math.abs(value - Number(item.quantity ?? 0)) < 0.0000001) {
   return;
  }

  const today = this.todayIsoDate();
  Swal.fire({
   title: `تعديل كمية (${item.category_name || ''})`,
   html:
    `<p class="text-right small mb-2">الكمية الجديدة: <strong>${value}</strong></p>` +
    `<label class="d-block text-right mb-1">تاريخ الحركة</label>` +
    `<input id="list-qty-date" type="date" class="swal2-input" style="margin:0 0 0.5rem" value="${today}" max="${today}">` +
   ...
   focusConfirm: false,
   preConfirm: () => {
    const dateEl = document.getElementById('list-qty-date') as HTMLInputElement | null;
  ...
  }).then((result: { isConfirmed: boolean; value?: string; isDismissed?: boolean }) => {
   if (!result.isConfirmed || !result.value) {
    input.value = String(item.quantity ?? 0);
    return;
   }
   this.category.updateQuantity(item.id, value, result.value).subscribe({
    next: (res: any) => {
     if (res.success) {
      item.quantity = res.quantity;
      if (res.total_price !== undefined && res.total_price !== null) {
      ...
      input.value = String(item.quantity ?? 0);
      Swal.fire({ icon: 'error', title: 'فشل تحديث الرصيد', text: res?.message || '' });
      this.search();
     }
    },
    error: (err) => {
     input.value = String(item.quantity ?? 0);
     Swal.fire({ icon: 'error', title: 'فشل تحديث الرصيد', text: err?.error?.message || '' });
     this.search();
    },
   });
  });
