...
export class ListCategoriesComponent {
  ...
  this.category.searchCategories(this.pageSize, this.page + 1, this.param).subscribe((data: any) => {
    this.allCategories = data.data; // نخزن النسخة الأصلية
    this.applyFilters();
    this.length = data.total;
    this.pageSize = data.per_page;
  });
...
applyFilters() {
  ...
  // ------------------------
// دالة لتحديد لون الرصيد
getQuantityColor(quantity: number): string {
  if (quantity === 0) return '#ffcccc';   // أحمر
  if (quantity < 10) return '#ffe5b4';    // برتقالي
  return '#ccffcc';                        // أخضر
}

// تحديث الرصيد عند التعديل
updateQuantity(item: any, event: Event) {
 ...
 this.category.updateQuantity(item.id, value).subscribe((res: any) => {
  if (res.success) {
    item.quantity = res.quantity; // الرقم اللي اتخزن فعليًا
  } else {
    Swal.fire({ icon: 'error', title: 'فشل تحديث الرصيد' });
    this.search(); // لإرجاع الرقم القديم
  }
});

}
onQuantityFilterChange() {
  this.search(); // أولًا نجيب البيانات كاملة

  if (!this.selectedQuantityFilter) return; // الكل
