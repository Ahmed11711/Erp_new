...
export class AddCategoryComponent implements OnInit {
 ...
 addCategory(data: any) {
  ...
  if (data.value.unit !== '' && data.value.unit != null) {
   formData.append('measurement_id', data.value.unit);
  }
  if (data.value.production !== '' && data.value.production != null) {
  ...
  const itemCode = (this.itemCode || data.value.item_code || '').toString().trim();
 ...
 onWarehouseChange(event: any) {
  ...
  this.previewNextItemCode();
 }

 private previewNextItemCode() {
  if (!this.warehouse) {
   return;
  }
  this.category.nextItemCode(this.warehouse).subscribe({
   next: (res) => {
    const next = (res?.item_code ?? '').toString().trim();
    if (!next) {
     return;
    }
    const current = (this.itemCode ?? '').toString().trim();
    if (!current || current === this.lastAutoItemCode) {
     this.lastAutoItemCode = next;
     this.itemCode = next;
    }
   },
   error: () => { },
  ...
  * لمخزن مستلزمات التشغيل نترك الحقل فارغاً (اختياري).
  */
 private syncSelectModel(controlName: 'unit' | 'production', rows: any[] | undefined, idKey: string) {
  setTimeout(() => {
   ...
   if (!ctrl) return;
   if (this.isOperatingSuppliesWarehouse || !rows?.length) {
    ctrl.setValue('');
