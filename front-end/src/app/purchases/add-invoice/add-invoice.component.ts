...
export class AddInvoiceComponent implements OnInit {
  ...
  products:any[] = [];
  categories : any[] = [];
  /** مخزن بنود الشراء: مواد خام أو مستلزمات تشغيل */
  purchaseWarehouse: 'مخزن مواد خام' | 'مستلزمات تشغيل وأدوات تشغيل' = 'مخزن مواد خام';
  readonly purchaseWarehouseOptions = [
    { value: 'مخزن مواد خام' as const, label: 'مخزن مواد خام' },
    { value: 'مستلزمات تشغيل وأدوات تشغيل' as const, label: 'مستلزمات تشغيل وأدوات تشغيل' },
  ];
  productions: Array<{ id: number; production_line?: string; warehouse?: string }> = [];
  lineProductionId: number | null = null;
  suppliers : any[] = [];
  shippingReps: any[] = [];
  ...
  /** يُرسل مع فاتورة المشتريات لربط السطر بصف واحد في categories */
  categoryId: number | null = null;

  loadPurchaseCategories(): void {
    this.cat.getCatBywarehouse(this.purchaseWarehouse).subscribe({
      next: (res: any) => {
  ...
  get isOperatingSuppliesPurchase(): boolean {
    return this.purchaseWarehouse === 'مستلزمات تشغيل وأدوات تشغيل';
  }
  ...
  addToTabel(){
    ...
    this.products.push({
      ...
      product_unit: (this.productUnit || '').toString().trim() || '—',
      category_id: this.categoryId,
      warehouse: this.purchaseWarehouse,
      production_id: this.isOperatingSuppliesPurchase ? this.lineProductionId : null,
    });
    this.calc();
