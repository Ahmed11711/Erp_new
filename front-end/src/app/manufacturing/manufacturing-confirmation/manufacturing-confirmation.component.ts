...
export class ManufacturingConfirmationComponent implements OnInit, OnDestroy {
  ...
  private loadAdditions(): void {
    this.manufacturingService.listAdditions().subscribe({
      next: (rows) => {
        this.more = (Array.isArray(rows) ? rows : []).map((row) => ({
          ...row,
  ...
  quantityFun(e: Event) {
    ...
    const previousQty = Number(this.quantity) || 1;
    if (t.value === '') {
      this.quantity = 1;
    } else {
      this.quantity = Number(t.value);
    }
    this.rescaleOrResetConsumptionForQuantity(previousQty);
    this.recalcTotal();
    this.scheduleConsumptionPreview();
  }

  private rescaleOrResetConsumptionForQuantity(previousQty: number): void {
    if (!this.consumptionLines.length || !(previousQty > 0) || !(this.quantity > 0)) {
      this.consumptionLines = [];
      return;
    }
    if (!this.hasCustomizedConsumption) {
      this.consumptionLines = [];
      return;
    }
    const ratio = this.quantity / previousQty;
    if (Math.abs(ratio - 1) < 1e-9) {
      return;
    }
    for (const line of this.consumptionLines) {
      line.quantity = Math.round(Number(line.quantity) * ratio * 1e6) / 1e6;
      line.default_quantity = Math.round(Number(line.default_quantity) * ratio * 1e6) / 1e6;
      line.line_cost = Math.round(Number(line.quantity) * Number(line.unit_cost) * 10000) / 10000;
      line.sufficient = Number(line.available_quantity) + 0.00001 >= Number(line.quantity);
  ...
  private recalcTotal() {
    this.total = Number(this.productPrice) * Number(this.quantity) + Number(this.additionCost || 0);
  }
  ...
  applyActualMaterialCost(): void {
    this.total = Math.round((Number(this.consumptionTotalCost) + Number(this.additionCost || 0)) * 10000) / 10000;
  ...
  moreChange(event: { cost?: number } | null) {
    this.additionCost = Number(event?.cost ?? 0);
    this.recalcTotal();
  }
  ...
  private buildConsumptionOverridesForRequest(): Array<{
    bom_item_id: number;
    ...
    return this.buildConsumptionPayload();
  }
  ...
  submit() {
    ...
    const consumptionLines = this.buildConsumptionOverridesForRequest();
    if (consumptionLines) {
